...
\title{Spatio-Temporal Areal Unit Modelling in \proglang{R} with Conditional Autoregressive Priors Using the \pkg{CARBayesST} Package}
...
Latent structure model - spatial and temporal main effects and an 
interaction
...
The posterior median sales rate $\{\theta_{kt}\}$ is computed and then plotted for the 6 odd numbered years using the code below, where the colour scale is the same as for Figure \ref{salesmap}. The first 3 lines create a \code{data.frame} object of estimated sales rates, while the fourth line adds these sales rate data to the \code{SpatialPolygonsDataFrame} object. Finally, the last line plots the estimated sales rates for odd numbered years.
