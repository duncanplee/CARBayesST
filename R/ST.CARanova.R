ST.CARanova <- function(formula, family, data=NULL,  trials=NULL, W, interaction=TRUE, burnin, n.sample, thin=1, prior.mean.beta=NULL, prior.var.beta=NULL, prior.nu2=NULL, prior.tau2=NULL, fix.rho.S=FALSE, rho.S=NULL, fix.rho.T=FALSE, rho.T=NULL, MALA=TRUE, verbose=TRUE)
{
    ...
    {
        if(!is.null(trials)) stop("you do not need a trials arugment as a binomial model was not specified", call.=FALSE)
        model <- poisson.CARanova(formula=formula, data=data,  W=W, interaction=interaction, burnin=burnin, n.sample=n.sample, thin=thin, prior.mean.beta=prior.mean.beta, prior.var.beta=prior.var.beta, prior.tau2=prior.tau2, fix.rho.S=fix.rho.S, rho.S=rho.S, fix.rho.T=fix.rho.T, rho.T=rho.T, MALA=MALA, verbose=verbose)          
    }else
    {
