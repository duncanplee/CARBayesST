binomial.CARanova <- function(formula, data=NULL, trials, W, interaction=TRUE, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, fix.rho.S=FALSE, rho.S=NULL, fix.rho.T=FALSE, rho.T=NULL, MALA=TRUE, verbose=TRUE)
{
##############################################
...
#### Create the MCMC samples    
    for(j in 1:n.sample)
    {
        ...
        if(MALA)
        {
        temp2 <- binomialcarupdateMALA(D.triplet, D.begfin, D.triplet.sum, N, delta, tau2.delta, Y.mat.trans.miss, failures.mat.trans.miss, trials.mat.trans, proposal.sd.delta, lambda, delta.offset, K, rep(1,K), which.miss.mat.trans)
        }else
        {
        temp2 <- binomialcarupdateRW(D.triplet, D.begfin, D.triplet.sum, N, delta, tau2.delta, Y.mat.trans.miss, failures.mat.trans.miss, proposal.sd.delta, lambda, delta.offset, K, rep(1,K), which.miss.mat.trans)
        }
        delta <- temp2[[1]]
        ...
        accept[6] <- accept[6] + N      
            ...
            if(MALA)
            {
            temp5 <- binomialindepupdateMALA(N.all, gamma, tau2.gamma, Y.miss, failures.miss, trials, proposal.sd.gamma, gamma.offset.vec, which.miss)
            }else
            {
            temp5 <- binomialindepupdateRW(N.all, gamma, tau2.gamma, Y.miss, failures.miss, proposal.sd.gamma, gamma.offset.vec, which.miss)
            }
            gamma <- temp5[[1]]
