poisson.CARlinear <- function(formula, data=NULL, W, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.mean.alpha=NULL, prior.var.alpha=NULL, prior.tau2=NULL, fix.rho.slo=FALSE, rho.slo=NULL, fix.rho.int=FALSE, rho.int=NULL, MALA=TRUE, verbose=TRUE)
{
#### Verbose
...
#### Create the MCMC samples    
    ...
    delta.offset <- offset.mat + regression.mat + phi.mat +  alpha * time.mat
        if(MALA)
        {
        temp2 <- poissoncarupdateMALA(W.triplet, W.begfin, W.triplet.sum, K, delta, tau2.delta,Y.mat.miss, proposal.sd.delta, lambda, delta.offset, N, time, which.miss.mat)
        }else
        {
        temp2 <- poissoncarupdateRW(W.triplet, W.begfin, W.triplet.sum, K, delta, tau2.delta,Y.mat.miss, proposal.sd.delta, lambda, delta.offset, N, time, which.miss.mat)
        }
    delta <- temp2[[1]]
        if(lambda <1)
        {
