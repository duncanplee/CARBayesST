poisson.CARar <- function(formula, data=NULL, W, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, fix.rho.S=FALSE, rho.S=NULL, fix.rho.T=FALSE, rho.T=NULL, MALA=TRUE, verbose=TRUE)
{
##############################################
...
#### Create the MCMC samples
    ...
    den.offset <- rho * W.triplet.sum + 1 - rho
        if(MALA)
        {
        temp1 <- poissonarcarupdateMALA(W.triplet, W.begfin, W.triplet.sum,  K, N, phi.mat, tau2, gamma, rho, Y.mat.miss, proposal.sd.phi, phi.offset, den.offset, which.miss.mat)      
        }else
        {
        temp1 <- poissonarcarupdateRW(W.triplet, W.begfin, W.triplet.sum,  K, N, phi.mat, tau2, gamma, rho, Y.mat.miss, proposal.sd.phi, phi.offset, den.offset, which.miss.mat)      
        }
    phi.temp <- temp1[[1]]
