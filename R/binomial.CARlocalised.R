...
#### Create the MCMC samples  
    ...
    ####################
        ...
        offset.temp <- offset + as.numeric(mu) + as.numeric(phi.mat)   
            if(p>2)
            {
            temp <- binomialbetaupdateMALA(X.standardised, N.all, p, beta, offset.temp, Y, failures, trials, prior.mean.beta, prior.var.beta, which.miss, n.beta.block, proposal.sd.beta, list.block)
            }else
            {
            temp <- binomialbetaupdateRW(X.standardised, N.all, p, beta, offset.temp, Y, failures, prior.mean.beta, prior.var.beta, which.miss, proposal.sd.beta)
            }
        beta <- temp[[1]]
    ...
    phi.offset <- mu + offset.mat + regression.mat
        if(MALA)
        {
        temp1 <- binomialarcarupdateMALA(W.triplet, W.begfin, W.triplet.sum,  K, N, phi.mat, tau2, gamma, 1, Y.mat, failures.mat, trials.mat, proposal.sd.phi, phi.offset, W.triplet.sum, which.miss.mat)      
        }else
        {
        temp1 <- binomialarcarupdateRW(W.triplet, W.begfin, W.triplet.sum,  K, N, phi.mat, tau2, gamma, 1, Y.mat, failures.mat, proposal.sd.phi, phi.offset, W.triplet.sum, which.miss.mat)      
        }
    phi.temp <- temp1[[1]]
